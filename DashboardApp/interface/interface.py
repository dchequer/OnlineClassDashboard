...
def meeting(meeting_name: str):
    ...
    meeting.subject_name = Subject.get_subject(
        owner_id=user_id, id=meeting.subject_id
    ).name
...
def deliverable(deliverable_title: str):
    user_id = session["user_id"]
    deliverable = Deliverable.get_deliverable_by_title(
        owner_id=user_id, title=deliverable_title
    )
    try:
        deliverable.subject_name = Subject.get_subject(
            owner_id=user_id, id=deliverable.subject_id
        ).name
    except AttributeError:
        deliverable.subject_name = None
    try:
        deliverable.meeting_name = Meeting.get_meeting(
            owner_id=user_id, id=deliverable.meeting_id
        ).name
    except AttributeError:
    ...
    return render_template(
        ...
        item_name=deliverable.title,
        parent_url="/core/deliverables",
